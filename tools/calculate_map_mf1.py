...
def main():
    ...
    candidate_models = [
        ...
        'ms-swift/output/export_v5_11968/open_ended/full_mapping',
    ...
    # 文本结果文件
    result_txt_file = os.path.join(result_dir, 'model_comparison.txt')
    
    # CSV结果文件
    result_csv_file = os.path.join(result_dir, 'model_comparison.csv')
    ...
    with open(result_txt_file, 'w') as txt_f, open(result_csv_file, 'w', newline='') as csv_f:
        ...
        for model in candidate_models:
            txt_f.write(f"Evaluating model: {model}\n")
            ...
            # 写入文本结果
            txt_f.write("VisDrone Results (Classes: {}):\n".format(visdrone_classes))
            txt_f.write("- mAPnc: {:.4f}\n".format(visdrone_map))
            txt_f.write("- mF1: {:.4f}\n".format(visdrone_mf1))
    ...
    print(f"Evaluation completed. Results saved to:")
    print(f"- Text file: {result_txt_file}")
    print(f"- CSV file: {result_csv_file}")
