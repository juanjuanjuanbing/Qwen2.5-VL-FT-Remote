...
class BBoxEvaluator:
    def __init__(
        ...
        self.instruction_processor = InstructionProcessor(
            model_api_base=self.detector.config.get("api_base", "http://localhost:8002/v1"),  # 从检测器配置获取 API 地址
            model_name=self.detector.config["model_name"]  # 与检测器使用同一模型
        )
...
if __name__ == "__main__":
    # Configuration
    IMAGE_LIST = "/home/zhiwei/FT_data/Qwen2.5-VL-FT-Remote/datasets/VLAD_Remote/test_image_list.txt"  # Path to text file with image paths
    INSTRUCTIONS_PATH = "/home/zhiwei/FT_data/Qwen2.5-VL-FT-Remote/tools/dataset_Remote/instructions_with_objects.json"  # Path to instructions JSON file
